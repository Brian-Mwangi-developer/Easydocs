...
export class JsonOutlineProvider implements vscode.TreeDataProvider<number>{
   private _onDidChangeTreeData: vscode.EventEmitter<number | undefined> = new vscode.EventEmitter<number | undefined>();
    ...
    private tree: json.Node | undefined; //json Node has type, value, children, comment, offset
    private text ='';
    ...
    getChildren(offset?: number): Thenable<number[]> {
		if (offset && this.tree) {
            ...
            if(node === undefined){ return Promise.reject(new Error(`Could not find json node at ${path}`));}
